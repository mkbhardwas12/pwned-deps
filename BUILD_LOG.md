...
### Sed-replace `YOUR_GH_USERNAME` → `mkbhardwas12`

User performed the placeholder substitution across the 6 files
that referenced it. Verified the sed-replace is clean (no
remaining occurrences) and dropped the now-stale "(placeholder
— sed-replace before publish)" parenthetical in the README
maintainer footer. Tests still 94 passed, 1 deselected.

### Hash-pinned `requirements.lock` (safety contract §2.6)

Generated by `pip-compile --generate-hashes` from a new
`requirements.in` (top-level deps with loose floors matching the
ranges in `pyproject.toml`). Outputs every direct + transitive
dep with two SHA-256 hashes (`.whl` + `.tar.gz`). 322 lines.

`Dockerfile.dev` now runs `pip install --require-hashes -r
requirements.lock`, which aborts the build if any package's
downloaded artifact doesn't match the recorded hash. Closes the
§2.6 follow-up.

`make pin-deps` regenerates the lockfile (uses `pip-tools==7.4.1`
inside the dev image with network on, output back into the host
file).

Version bumps that came with regeneration:
...
| pytest       | 8.3.3  | 8.4.2   |
| pytest-httpx | 0.32.0 | 0.35.0  |
| pyyaml       | 6.0.2  | 6.0.3   |
| rich         | 13.9.4 | 14.3.4  |
| ruff         | 0.7.4  | 0.15.12 |

One test required a click-API fix:
`CliRunner(mix_stderr=False)` was dropped in click 8.3 — stderr
is split by default now and read via `result.stderr`. Updated
`test_check_multiple_paths_skips_unrecognised` accordingly.

Live dogfood against the hash-pinned lockfile: 24 packages
scanned, 1 informational finding (`pytest@8.4.2
GHSA-6w46-j5rx-g56g` MEDIUM — applies upstream to 8.4.x too).
Exit 0; no malicious deps. Full sweep is green:

- `make verify-safety` → OK
- `make verify-safety-self-test` → caught planted `eval()`
- `make build` → exit 0 with `--require-hashes`
