...
class Advisory:
    ...
    @property
    def whole_package_malicious(self) -> bool:
        """GHSA ranges like ``> 0`` / ``>= 0`` mean every version is bad."""

        r = self.version_range.replace(" ", "")
        return r in (">0", ">=0", ">=0.0.0", ">0.0.0", "*", "")

    @property
    def likely_hijack(self) -> bool:
        """Established project + only specific versions flagged.

        Old spam packages (one version, published years ago, whole
        package malicious) are not hijacks even though they are old.
        """

        age = self.age_at_advisory
        return (
            age is not None
            and age >= HIJACK_MIN_AGE
            and (self.versions or 0) >= HIJACK_MIN_VERSIONS
            and not self.whole_package_malicious
        )
...
def render(advs: list[Advisory], since: datetime, now: datetime) -> str:
    ...
    lines += [
        ...
        f"registry first-publish ≥{HIJACK_MIN_AGE.days} days before the GHSA publish date, "
        f"≥{HIJACK_MIN_VERSIONS} released versions, and a version-specific affected range "
        "(not `> 0`)._",
    ]
    return "\n".join(lines) + "\n"
